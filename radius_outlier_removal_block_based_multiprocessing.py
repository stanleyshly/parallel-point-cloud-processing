...
def multi_run_wrapper(args):
   return helper_multiprocess_funct(*args)
def helper_multiprocess_funct(current, pcd_cp, block_x_length, block_y_length, block_z_length, radius, nb_neighbors):
	if (current.shape[0] != 0):
			...
			if (inner.shape[0] != 0):
				...
				#print(num_points)
				pcd = radial_remove(inner, outer, num_points, nb_neighbors=nb_neighbors, radius=radius)
				return pcd
			else:
				return None
	else:
...
def block_radial_remove_multiprocessing(pcd_cp, blocknumx=1, blocknumy=1, blocknumz=1, radius=1, nb_neighbors=1, cores=1):
	...
	print('Removing Outliers with Multiprocessing Block Based Method ... Processing:',str(block_start_array.shape[0]), "blocks")
	...
	for itr in (range(block_start_array.shape[0])):
		...
		array.append((current, pcd_cp, block_x_length, block_y_length, block_z_length, radius, nb_neighbors))
		#result = multi_run_wrapper((current, pcd_cp, block_x_length, block_y_length, block_z_length, radius, nb_neighbors))
		if (count %cores == 0):
...
def radial_remove(pcd, outer_pcd, num_points, nb_neighbors=0, radius=5):
